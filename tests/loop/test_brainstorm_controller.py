...
def test_drain_queue_processes_pending_item(tmp_path):
    ...
    with patch.object(bc_mod, "todo_queue") as mock_tq, \
         ...
         patch.object(bc_mod, "agent_runner") as mock_runner:
        mock_tq.next_pending.side_effect = [item, None]
        mock_runner.run_brainstorm_item.return_value = "session-abc"
        _ctrl()._drain_queue()
    mock_runner.run_brainstorm_item.assert_called_once_with(item, {"claude_cmd": "claude"})
...
def test_drain_queue_skips_when_session_id_is_none(tmp_path):
    ...
    with patch.object(bc_mod, "todo_queue") as mock_tq, \
         ...
         patch.object(bc_mod, "agent_runner") as mock_runner:
        mock_tq.next_pending.side_effect = [item, None]
        mock_runner.run_brainstorm_item.return_value = None
        _ctrl()._drain_queue()
...
def test_drain_queue_processes_multiple_items(tmp_path):
    ...
    with patch.object(bc_mod, "todo_queue") as mock_tq, \
        ...
        mock_runner.run_brainstorm_item.return_value = "sess"
        _ctrl()._drain_queue()
    assert mock_runner.run_brainstorm_item.call_count == 2
...
def test_drain_queue_breaks_loop_on_repeated_item(tmp_path):
    ...
    with patch.object(bc_mod, "todo_queue") as mock_tq, \
        ...
        mock_runner.run_brainstorm_item.return_value = None  # always fails
        _ctrl()._drain_queue()
    assert mock_runner.run_brainstorm_item.call_count == 1
...
def test_drain_queue_passes_config_to_agent_runner(tmp_path):
    item = {"id": "1", "title": "T", "description": "D", "project_dir": str(tmp_path)}
    with patch.object(bc_mod, "todo_queue") as mock_tq, \
         patch.object(bc_mod, "work_queue") as mock_wq, \
         patch.object(bc_mod, "agent_runner") as mock_runner:
    ...
    mock_runner.run_brainstorm_item.assert_called_once_with(
        item, {"claude_cmd": "/usr/local/bin/claude"}
    )
